...
import {
  LayoutDashboard, FileText, FolderOpen, CreditCard, Download, MessageSquare, Settings, LogOut, Menu, X, Bell, Map, Home, Building2, Store, User, HelpCircle
} from "lucide-react";
...
const navItems = [
  ...
  { icon: Download, label: "Approved Certificates", path: "/dashboard/certificates" },
  { icon: Map, label: "Street Registration", path: "/dashboard/street-registration" },
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-sidebar text-sidebar-foreground flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="flex items-center gap-3 px-5 h-16 border-b border-sidebar-border">
          ...
          <div>
            ...
            <span className="block text-xs text-sidebar-foreground/60">Ifo LGA</span>
          </div>
